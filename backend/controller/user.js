...
exports.signUp = (req, res, next) => {
  ...
  User
  .findOne({ email })    //Checking if the email exist
  .then(user => {
    if(user) res.status(409).json({ error: "The entered Email already exist!" })
    else {
      //Hashing the password
      bcrypt.hash(password, 10, (error, hash) => {
        if(error) res.status(500).json({ error })
        else {
          const userData = new User({_id: mongoose.Types.ObjectId(), email: email, password: hash, favouriteMovies: [] });
          userData.save()
          .then(() => { 
            ...
            res.status(201).json({ message: "The user has been signed up successfully!", userData, favouriteMovies: [] })
